...
static int send_subscribe_unsubscribe_message(int req_fifo_fd, enum OperationCode opcode, const char* key) {
...
int kvs_connect(const char* req_pipe_path, const char* resp_pipe_path, const char* notif_pipe_path, const char* registry_fifo, int* req_fifo_fd, int* resp_fifo_fd, int* notif_fifo_fd) {
  ...
  *req_fifo_fd = open(req_pipe_path, O_WRONLY);
  *resp_fifo_fd = open(resp_pipe_path, O_RDONLY);
  *notif_fifo_fd = open(notif_pipe_path, O_RDONLY | O_NONBLOCK);
  ...
  if (check_server_response(*resp_fifo_fd, &response_code)) {
    close(*req_fifo_fd);
    close(*resp_fifo_fd);
    close(*notif_fifo_fd);
    return 1;
...
int kvs_disconnect(int* req_fifo_fd, int* resp_fifo_fd) {
  ...
  if (write(*req_fifo_fd, message, sizeof(message)) == -1) {
    fprintf(stderr, "Failed to write on the registry named pipe (FIFO).\n");
    status = 1;
  } else {
    int response_code;
    // If the disconnect message to the request pipe is sent successfully verify server response.
    status = check_server_response(*resp_fifo_fd, &response_code);  
    // Print the server response
...
int kvs_subscribe(int* req_fifo_fd, int* resp_fifo_fd, const char* key) {
  // Send the subscribe message to the request pipe
  if (send_subscribe_unsubscribe_message(*req_fifo_fd, OP_CODE_SUBSCRIBE, key)) {
    return 1;
  }
...
int kvs_unsubscribe(int* req_fifo_fd, int* resp_fifo_fd, const char* key) {
  ...
  if (send_subscribe_unsubscribe_message(*req_fifo_fd, OP_CODE_UNSUBSCRIBE, key)) {
    return 1;
  }
